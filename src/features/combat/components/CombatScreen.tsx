        ...
        <div className={styles.combatResults}>
          <div className={styles.playerResults}>
            <div className={`${styles.playerSection} ${styles.player}`}>
              ...
              <div className={styles.diceResult}>
                🗡️ Attaque: {diceResults.playerAttack} + {currentPlayerCard.attackMod} ={' '}
                {combatResult.playerAttack}
              </div>
              <div className={styles.diceResult}>
                🛡️ Défense: {diceResults.playerDefense} + {currentPlayerCard.defenseMod} ={' '}
                {combatResult.playerDefense}
              </div>
              <div
                className={styles.damageText}
                style={{
                  color: combatResult.damageToPlayer > 0 ? '#FF6B6B' : '#4ADE80',
                }}
              >
            ...
            <div className={`${styles.playerSection} ${styles.ia}`}>
              <h3>🤖 Ennemi</h3>
              <div className={styles.diceResult}>
                🗡️ Attaque: {diceResults.enemyAttack} + {currentEnemyCard.attackMod} ={' '}
                {combatResult.enemyAttack}
              </div>
              <div className={styles.diceResult}>
                🛡️ Défense: {diceResults.enemyDefense} + {currentEnemyCard.defenseMod} ={' '}
                {combatResult.enemyDefense}
              </div>
              <div
                className={styles.damageText}
                style={{
          ...
          {combatFinished && (
            <div className={styles.finalResult}>
              {currentEnemyCard.currentHp <= 0 ? '🎉 VICTOIRE !' : '💀 DÉFAITE...'}
            </div>
          )}
        </div>
      )}
